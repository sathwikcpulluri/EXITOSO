    ...
    <div className="space-y-8 animate-fade-in pb-12 text-white max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {assessmentResult ? (
            (() => {
              const matchingSkills = Array.isArray(assessmentResult.matching_skills) ? assessmentResult.matching_skills : []
              const missingSkills = Array.isArray(assessmentResult.missing_skills) ? assessmentResult.missing_skills : []
              const skillGaps = Array.isArray(assessmentResult.skill_gaps) ? assessmentResult.skill_gaps : []
              const factors = Array.isArray(assessmentResult.factors) ? assessmentResult.factors : []
              const recommendations = Array.isArray(assessmentResult.recommendations) ? assessmentResult.recommendations : []
                  ...
                  <div className="p-6 rounded-2xl bg-gradient-to-r from-rose-950/40 via-neutral-900/90 to-orange-950/30 border border-white/10 shadow-[0_15px_35px_rgba(0,0,0,0.5)] space-y-6 animate-fade-in">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-white/[0.08]">
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          <Badge variant={getMatchBadgeVariant(assessmentResult.recommendation)}>
                            {(assessmentResult.recommendation || 'MATCH').toUpperCase()} MATCH
                          </Badge>
                          <span className="text-xs text-neutral-400">
                            Analysis confidence: Based on available profile data
                          </span>
                        </div>
                        <h2 className="text-2xl font-bold text-white tracking-tight">{assessmentResult.job_title || 'Target Role'}</h2>
                        <p className="text-xs text-neutral-400">{assessmentResult.company_name || 'Hiring Organization'}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <ScoreRing score={assessmentResult.overall_score || 0} size="lg" />
                      </div>
                    </div>

                    {/* Score Bars Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <ScoreBar label="Technical Skill Match" score={assessmentResult.technical_score || 0} />
                      <ScoreBar label="Experience Depth" score={assessmentResult.experience_score || 0} />
                      <ScoreBar label="Role Alignment" score={assessmentResult.role_alignment_score || 0} />
                      <ScoreBar label="Cultural & Team Fit" score={assessmentResult.cultural_score || 85} />
                    </div>
                  </div>
                  ...
                  <Card className="p-6 space-y-4 border-white/10">
                    ...
                    <div className="space-y-4 text-xs sm:text-sm text-neutral-300 leading-relaxed">
                      <p className="p-4 bg-white/[0.03] rounded-xl border border-white/[0.08] text-neutral-300 leading-relaxed">
                        {assessmentResult.explanation || 'Detailed skill and role analysis completed.'}
                      </p>
                    ...
                    <Card className="p-6 border-white/10">
                      <CardHeader>
                        <CardTitle className="text-sm font-bold text-emerald-400 flex items-center gap-2">
                          <CheckCircle2 className="h-4 w-4" /> Matched Skills ({matchingSkills.length})
                        </CardTitle>
                      </CardHeader>
                      <div className="mt-2">
                    ...
                    <Card className="p-6 border-white/10">
                      <CardHeader>
                        <CardTitle className="text-sm font-bold text-amber-400 flex items-center gap-2">
                          <AlertTriangle className="h-4 w-4" /> Identified Skill Gaps ({missingSkills.length})
                        </CardTitle>
                      </CardHeader>
